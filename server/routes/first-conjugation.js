...
import { FIRST_CONJUGATION_RULE } from '../utils/first-conjugation-rule.js';
...
router.get('/rule', (req, res) => {
  res.json(FIRST_CONJUGATION_RULE);
});

// GET random verb exercise
router.get('/random', (req, res) => {
  const verbs = Object.keys(FIRST_CONJUGATION_RULE.verbs);
  const randomVerb = verbs[Math.floor(Math.random() * verbs.length)];
  const verbData = FIRST_CONJUGATION_RULE.verbs[randomVerb];
...
router.get('/exercise/:verb/:person', (req, res) => {
  const verbData = FIRST_CONJUGATION_RULE.verbs[req.params.verb];
  if (!verbData) {
...
router.post('/result', async (req, res) => {
  try {
    ...
    await database.run(
      ...
      [userId, 1, `first_conj_${verb}_${person}`, isCorrect ? 1 : 0]
    );
    ...
    const stats = await database.get(`
      ...
      FROM user_progress
      WHERE user_id = ? AND exercise_type LIKE 'first_conj_%'
    `, [userId]);
